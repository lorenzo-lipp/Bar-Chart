...
function renderData(data) {
	...
	const handleMaxInput = (e) => {
		...
		d3.select('.slider-progress')
			.style('width', `${(700 * (e.target.value - selectedMin) / totalYears) - 10}px`);
	}
	...
	d3.select('#graph')
		...
		.style('left', `calc(50% + ${((selectedMin - 1947) / totalYears) * 690}px - 352px)`)
		.text(selectedMin);
	...
	d3.select('#graph')
		...
		.attr('id', 'slider-max')
		.attr('class', 'slider-text')
		.style('left', `calc(50% + ${-((2015 - selectedMax) / totalYears) * 690}px + 332px)`)
		.text(selectedMax);
...
function clear() {
	d3.select('#graph').html(`<svg width="${w}" height="${h}" viewBox="0 0 ${w} ${h * 0.96}"></svg>`);
}
